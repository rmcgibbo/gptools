...
class Matern52Kernel(Kernel):
    ...
    **kwargs
    ...
    def __call__(self, Xi, Xj, ni, nj, hyper_deriv=None, symmetric=False):
        ...
        Xi = scipy.asarray(Xi, dtype=scipy.float64, order='c')
        Xj = scipy.asarray(Xj, dtype=scipy.float64, order='c')
    ...
    **kwargs
    ...
    def __init__(self, num_dim=1, **kwargs):
        ...
        for i in range(num_dim):
            param_names.append('l_%d' % (i+1))
            param_names.append('alpha_%d' % (i+1))
            param_names.append('beta_%d' % (i+1))
        super(BetaWarpedMatern52Kernel, self).__init__(num_dim=num_dim,
                                             num_params=3*num_dim + 1,
                                             param_names=param_names,
                                             **kwargs)
    def __call__(self, Xi, Xj, ni, nj, hyper_deriv=None, symmetric=False):
        """Evaluate the covariance between points `Xi` and `Xj` with derivative order `ni`, `nj`.
        ...
        if hyper_deriv is not None:
            raise NotImplementedError("Hyperparameter derivatives have not been implemented!")
        if scipy.any(ni > 0) or scipy.any(nj > 0):
        ...
        ni = scipy.array(ni, dtype=scipy.int32, order='c')
        nj = scipy.array(nj, dtype=scipy.int32, order='c')
        var = scipy.square(self.params[1::3])
        alpha = self.params[2::3]
        beta = self.params[3::3]

        wXi = scipy.special.betainc(alpha, beta, Xi)
        wXj = scipy.special.betainc(alpha, beta, Xj)

        value = _matern52(wXi, wXj, ni, nj, var)
        return self.params[0]**2 * value
